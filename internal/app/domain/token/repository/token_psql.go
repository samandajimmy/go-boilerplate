...
func (m *psqlTokenRepository) RCreate(c echo.Context, accToken *model.AccountToken) error {
	var lastID int64
	now := time.Now()
...
func (m *psqlTokenRepository) RGetByUsername(c echo.Context, accToken *model.AccountToken) error {
...
func (m *psqlTokenRepository) RUpdateToken(c echo.Context, accToken *model.AccountToken) error {
	var ID int64
	now := time.Now()
	tokenExp := now.Add(stringToDuration(os.Getenv(`JWT_TOKEN_EXP`)) * time.Hour)
	token, err := createJWTToken(accToken, now, tokenExp)
...
func (m *psqlTokenRepository) RUpdateAllAccountTokenExpiry() error {
